...
# h2020
h2020_cr_df <- readr::read_csv("data/h2020_cr_df.csv")
h2020_cr_all <- h2020_cr_df %>%
    group_by(issn_l) %>%
    summarise(h2020_n = sum(h2020_n))
jn_ind_table <- inner_join(jn_ind_table, h2020_cr_all, by = "issn_l") %>%
    mutate(h2020 = h2020_n / articles) %>%
    select(-h2020_n)
```
...
get_ec_color <- function(score) {
    ec_pal <- function(x) rgb(colorRamp(c("#445c91", "#003399"))(x), maxColorValue = 255)
    normalized <- (score - min(score)) / (max(score) - min(score))
    ec_pal(normalized)
}
...
jn_df <- jn_ind_table %>%
    ungroup() %>%
    mutate(across(c(hybrid, green, h2020), ~ round(.x, 3) * 100)) %>%
    mutate(across(c(hybrid, green, h2020), ~ replace_na(., 0))) %>%
    mutate(
        hybrid_color = get_hybrid_color(hybrid),
        green_color = get_green_color(green),
        ec_color = get_ec_color(h2020)
    ) %>%
    select(journal, articles, hybrid,
      green, h2020, issn_l, vertrag, hybrid_color, green_color, ec_color)
```
...
jn_tbl <- reactable::reactable(shared_tbl,
    ...
    columns = list(
        ...
        h2020 = colDef(
            ...
            # are hard to compare, apply a color scale as well.
            cell = JS("function(cellInfo) {
        const sliceColor = cellInfo.row['ec_color']
        const sliceLength = 2 * Math.PI * 24
        const sliceOffset = sliceLength * (1 - cellInfo.value / 100)
        const donutChart = (
          '<svg width=60 height=60 style=\"transform: rotate(-90deg)\" focusable=false>' +
        ...
        ec_color = colDef(show = FALSE)
    ),
    columnGroups = list(
    colGroup(name = "Open Access", columns = c("hybrid", "green")),
    colGroup(name = "Funding", columns = c("h2020"))
  ),
...
htmltools::div(class = "agreement-tbl",
 ...
 # select_filter("filter_title", "Search contract", shared_tbl, "vertrag"),
  select_filter("filter_title", "Select Contract", shared_filter, "vertrag"),
    jn_tbl,
  htmltools::div(class = "agreement-footer", "Data sources: Open Access Monitor: Zeitschriftenlisten (v1), Crossref, Unpaywall, OpenAIRE Research Graph")
)
```
